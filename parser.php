...
/**
 *   Changes to the code by Tiago Bilou
 * 
 */
...
function parse($text){
  ...
  // pickup titles of existing pages from blog posts and link them appropriately
  wp_pages($table,$text);
...
function format_headlines(&$table,&$hltable,&$text){
  ...
  foreach($hltable as $hl){
    ...
    $headline  .= '<h'.$hl['level'].' class=\'wiki\'>';
    $headline  .= $hl['name'];
    $headline  .= '</h'.$hl['level'].'>';
...
function smileys(&$table,&$text){
  $smileys = file(ABSPATH."/wp-content/plugins/dokupress/smileys.conf");
...
/**
 * Pick up post and page titles and link back to them
 * Based upon the acronyms() function code below
 *
 * @author  Paulo Andre <prla@alinex.org>
 */
...
function acronyms(&$table,&$text){
  $acronyms = file(ABSPATH."wp-content/plugins/dokupress/acronyms.conf");
...
function customs($text){
  $reps = file (ABSPATH.'wp-content/plugins/dokupress/custom.conf');
  foreach($reps as $rep){
  ...
  switch ($type){
    ...
    case 'code':
      if(empty($option)){
        ...
        require_once(ABSPATH."wp-content/plugins/dokupress/geshi.php");
        $geshi = new GeSHi($text, strtolower($option), ABSPATH."wp-content/plugins/dokupress/geshi/");
